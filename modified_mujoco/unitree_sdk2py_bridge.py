...
class UnitreeSdk2Bridge:
    ...
    def __init__(self, mj_model, mj_data, sim_lock=None):
        self.mj_model = mj_model
        self.mj_data = mj_data
        self.sim_lock = sim_lock  # Lock for synchronizing with simulation thread
        ...
        # LowCmd message storage with lock for thread-safe access
        self.low_cmd_msg = None
        ...
        # Control thread for fixed frequency motor control
        # This ensures control runs at fixed frequency (e.g., 500Hz or 1000Hz)
        # regardless of lowcmd publish frequency
        control_freq = getattr(config, 'CONTROL_FREQ', 500)  # Default 500Hz
    ...
    def LowCmdHandler(self, msg: LowCmd_):
        """
        Callback function when lowcmd message is received.
        This only stores the message, actual control is applied in ControlLoop.
        """
    ...
    def ControlLoop(self):
        """
        Fixed frequency control loop that applies lowcmd at the specified control frequency.
        This ensures consistent motor control frequency regardless of lowcmd publish rate.
        """
        if self.mj_data is None:
            return
        
        # Get the latest lowcmd message
        with self.low_cmd_lock:
            if not self.low_cmd_received or self.low_cmd_msg is None:
                return
            # Copy reference to message (message itself is immutable from our perspective)
            msg = self.low_cmd_msg
        
        # Apply control command to motors
        # Use simulation lock if available to ensure thread safety
        if self.sim_lock is not None:
            ...
            # If no lock provided, access directly (less safe but may be needed)
            for i in range(self.num_motor):
                self.mj_data.ctrl[i] = (
                    msg.motor_cmd[i].tau
                    + msg.motor_cmd[i].kp
    ...
    def PublishLowState(self):
        if self.mj_data != None:
            self.low_state.tick = int(self.mj_data.time / self.dt)
            for i in range(self.num_motor):
